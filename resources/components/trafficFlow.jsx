...
class TrafficFlow extends React.Component {
    constructor(props) {
        ...
        this.ws.onopen = () => {
            this.ws.send("get_traffic");
        };
        this.ws.onmessage = (evt) => {
            this.traffic.clientUpdateTime = Date.now();

            this.updateData(JSON.parse(String.fromCharCode.apply(null, new Uint8Array(evt.data))));
        ...
        this.ws.onclose = () => {
            ...
            this.updateData(this.traffic);
            alert("Communication error: WebSocket - Connection closed");
        };
    ...
    objectHighlighted = (highlightedObject) => {
        ...
        this.setState({
            ...
            objectToHighlight: highlightedObject ? highlightedObject.getName() : undefined,
            searchTerm: '',
            matches: {total: -1, visible: -1},
            redirectedFrom: undefined
        });
        ...
        request.get('/api/traffic/data')
            ...
            .end((err, res) => {
                if (res && res.status === 200) {
                    ...
                    alert("Communication error: Back-end not send initial structure");
                }
            ...
            <div className="vizceral-container">
                {this.state.redirectedFrom ?
                    <Alert onDismiss={this.dismissAlert}>
                        <strong>{this.state.redirectedFrom.join('/') || '/'}</strong> does not exist, you were
                        redirected to <strong>{this.state.currentView.join('/') || '/'}</strong> instead
                    </Alert>
                    : undefined}
                <div className="subheader">
                    ...
                    <div style={{float: 'right', paddingTop: '4px'}}>
                        {(!globalView && matches) &&
                        <Locator changeCallback={this.locatorChanged} searchTerm={this.state.searchTerm}
                                 matches={matches} clearFilterCallback={this.filtersCleared}/>}
                        ...
                        <a role="button" className="reset-layout-link" onClick={this.resetLayoutButtonClicked}>Reset
                            Layout</a>
                    </div>
                </div>
